...
class FusionService extends FusionService_Original implements \Neos\Flow\ObjectManagement\Proxy\ProxyInterface {
    ...
    protected function Flow_Aop_Proxy_buildMethodsAndAdvicesArray()
    {
        ...
        $this->Flow_Aop_Proxy_targetMethodsAndGroupedAdvices = array(
            'getMergedFusionObjectTree' => array(
                'Neos\Flow\Aop\Advice\AroundAdvice' => array(
                    new \Neos\Flow\Aop\Advice\AroundAdvice('Neos\Neos\Aspects\FusionCachingAspect', 'cacheGetMergedFusionObjectTree', $objectManager, NULL),
                ),
            ),
        );
    }
     ...
     * @throws \Neos\Fusion\Exception
     */
    public function getMergedFusionObjectTree(\Neos\ContentRepository\Domain\Model\NodeInterface $startNode)
    {

        if (isset($this->Flow_Aop_Proxy_methodIsInAdviceMode['getMergedFusionObjectTree'])) {
            $result = parent::getMergedFusionObjectTree($startNode);

        } else {
            $this->Flow_Aop_Proxy_methodIsInAdviceMode['getMergedFusionObjectTree'] = true;
            try {
                ...
                $adviceChains = $this->Flow_Aop_Proxy_getAdviceChains('getMergedFusionObjectTree');
                $adviceChain = $adviceChains['Neos\Flow\Aop\Advice\AroundAdvice'];
                $adviceChain->rewind();
                $joinPoint = new \Neos\Flow\Aop\JoinPoint($this, 'Neos\Neos\Domain\Service\FusionService', 'getMergedFusionObjectTree', $methodArguments, $adviceChain);
                $result = $adviceChain->proceed($joinPoint);
                $methodArguments = $joinPoint->getMethodArguments();

            } catch (\Exception $exception) {
                unset($this->Flow_Aop_Proxy_methodIsInAdviceMode['getMergedFusionObjectTree']);
                throw $exception;
            }
            unset($this->Flow_Aop_Proxy_methodIsInAdviceMode['getMergedFusionObjectTree']);
        }
  ...
  2 => 'nodeTypeManager',
  3 => 'packageManager',
  4 => 'autoIncludeConfiguration',
);
    }
}
# PathAndFilename: C:\Bitnami\neos-4.1.7-neu\apps\neos\htdocs\Packages\Application\Neos.Neos\Classes\Domain\Service\FusionService.php
#